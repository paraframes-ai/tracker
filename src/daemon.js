...
async function writeToDisk(rel) {
  ...
  if (cur === content) return; // already current (also swallows our own echo)
...
function connectPresence(provider) {
  const aw = provider.awareness;
  aw.setLocalStateField('user', { name: config.name });
  let lastPeers = '';
  aw.on('change', () => {
    const peers = [...aw.getStates().entries()]
...
async function main() {
  log(`ParaFrames Live`);
  log(`room=${config.room} relay=${config.relay} name=${config.name}`);
  log(`root=${config.root}`);

  const provider = new WebsocketProvider(config.relay, config.room, doc, {
