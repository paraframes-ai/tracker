...
var emitWithoutRejectionCapture = function(type, ...args2) {
  if (type === "error")
  ...
  for (let i = 0, { length: length2 } = maybeClonedHandlers;i < length2; i++) {
...
var emitWithRejectionCapture = function(type, ...args2) {
  if (type === "error")
    return emitError(this, args2);
  var { _events: events } = this;
  if (events === undefined)
...
function overflowWarning(emitter, type, handlers) {
  ...
  let warn2 = Error(`Possible EventEmitter memory leak detected. ${handlers.length} ${String(type)} listeners added to [${emitter.constructor.name}]. Use emitter.setMaxListeners() to increase limit`);
  warn2.name = "MaxListenersExceededWarning", warn2.emitter = emitter, warn2.type = type, warn2.count = handlers.length, console.warn(warn2);
}
...
function validateNumber(value, name, min2, max2) {
  ...
  if (min2 != null && value < min2 || max2 != null && value > max2 || (min2 != null || max2 != null) && Number.isNaN(value))
    throw ERR_OUT_OF_RANGE(name, `${min2 != null ? `>= ${min2}` : ""}${min2 != null && max2 != null ? " && " : ""}${max2 != null ? `<= ${max2}` : ""}`, value);
}
...
function addAbortListener(signal, listener) {
  ...
  if (signal.aborted)
    ...
    signal.addEventListener("abort", listener, { __proto__: null, once: true }), removeEventListener2 = () => {
...
function encodeSealed(type, sender, sealed) {
...
function decodeSealed(payload) {
  if (payload.length < 1)
    throw new Error("truncated frame");
  const nameLen = payload[0];
  if (payload.length < 1 + nameLen)
    throw new Error("truncated sender");
  return {
    sender: new TextDecoder().decode(payload.subarray(1, 1 + nameLen)),
    sealed: payload.subarray(1 + nameLen)
  };
}
var decodeControl = (payload) => JSON.parse(new TextDecoder().decode(payload));

// src/crypto.js
var KEY_BYTES = 32;
var NONCE_BYTES = 12;
function fromBase64Url(str) {
  const bytes = new Uint8Array(Buffer.from(str.replace(/-/g, "+").replace(/_/g, "/"), "base64"));
  if (bytes.length !== KEY_BYTES) {
    throw new Error(`room key must be ${KEY_BYTES} bytes, got ${bytes.length}`);
  }
  return bytes;
}
function importRoomKey(rawKey) {
  return crypto.subtle.importKey("raw", rawKey, { name: "AES-GCM" }, false, [
    "encrypt",
    "decrypt"
  ]);
}
var aad = (frameType, owner, session, sender) => new TextEncoder().encode(`${frameType}\x00${owner}\x00${session}\x00${sender}`);
async function seal(key, frameType, ctx, plaintext) {
  const nonce = crypto.getRandomValues(new Uint8Array(NONCE_BYTES));
  const ct = await crypto.subtle.encrypt({ name: "AES-GCM", iv: nonce, additionalData: aad(frameType, ctx.owner, ctx.session, ctx.sender) }, key, plaintext);
  const out = new Uint8Array(NONCE_BYTES + ct.byteLength);
  out.set(nonce, 0);
  out.set(new Uint8Array(ct), NONCE_BYTES);
  return out;
}
async function open(key, frameType, ctx, payload) {
  if (payload.length <= NONCE_BYTES)
    throw new Error("frame too short");
  const nonce = payload.subarray(0, NONCE_BYTES);
  const ct = payload.subarray(NONCE_BYTES);
  const pt = await crypto.subtle.decrypt({ name: "AES-GCM", iv: nonce, additionalData: aad(frameType, ctx.owner, ctx.session, ctx.sender) }, key, ct);
...
var CONNECTION_STABLE_MS = 5000;

class EncryptedProvider extends EventEmitter {
  constructor({
    relay,
    owner,
    session,
    ...
    this.awareness = awareness ?? new Awareness(doc2);
    ...
    this._onDocUpdate = (update, origin) => {
      ...
      this._sendSealed(FRAME.CONTENT, toUint8Array(enc));
    };
  ...
  _open() {
    ...
    const ws = browserLike ? new Impl(this._url(), [`bearer.${this.token}`]) : new Impl(this._url(), { headers: { Authorization: `Bearer ${this.token}` } });
    ...
    const onOpen = () => {
      ...
      this.pingTimer = setInterval(() => {
        if (ws.readyState === 1)
          ws.send(encodeFrame(FRAME.PING));
      }, PING_INTERVAL_MS);
    };
    ...
    if (browserLike) {
      ...
      ws.addEventListener("error", () => this.emit("status", { status: "error", reason: "websocket error" }));
    } else {
      ws.on("open", onOpen);
  ...
  _afterClose(code, reason, openedAt) {
    ...
    if (reason === "maxFrameBytes") {
      this.emit("fatal", {
        code,
        reason: "this project is too large for the relay to sync in one message " + "(initial sync exceeded the relay frame limit)"
      });
      return;
    }
    if (this.destroyed)
  ...
  async _onMessage(data) {
    ...
    if (type !== FRAME.CONTENT && type !== FRAME.AWARENESS)
      return;
    const { sender, sealed } = decodeSealed(payload);
    if (sender === this.username)
      return;
    if (this.badSenders.has(sender))
      return;
    let plaintext;
    try {
    ...
    if (type === FRAME.AWARENESS) {
      applyAwarenessUpdate(this.awareness, plaintext, this);
      return;
    }
    const decoder = createDecoder(plaintext);
    const reply = createEncoder();
    const msgType = readSyncMessage(decoder, reply, this.doc, this);
    if (length(reply) > 0) {
      this._sendSealed(FRAME.CONTENT, toUint8Array(reply));
    }
    if (msgType === messageYjsSyncStep2)
      this._markSynced();
  }
  _onControl(msg) {
    if (msg.type === "hello") {
  ...
  _sendSealed(type, plaintext) {
    if (!this.key || this.ws?.readyState !== 1)
      return;
    seal(this.key, type, this.ctxFor(this.username), plaintext).then((sealed) => {
...
// src/browser/viewer.js
var KEY = "tracker.dashboard.token";
...
var state = { files: new Map, selected: null, peers: [], status: "connecting" };
function renderTree() {
  const names = [...state.files.keys()].sort();
  $2("count").textContent = names.length ? `${names.length} files` : "no files yet";
  $2("tree").innerHTML = names.map((n) => `<li${n === state.selected ? ' class="sel"' : ""} data-f="${esc(n)}"><span>${esc(n)}</span></li>`).join("");
  for (const li of $2("tree").querySelectorAll("li")) {
    li.onclick = () => {
...
function renderFile() {
  ...
  if (!name) {
    $2("file").innerHTML = '<div class="empty">Select a file to view it.</div>';
    return;
  }
  const text2 = state.files.get(name) ?? "";
  const lines = text2.split(`
`);
  $2("file").innerHTML = `<div class="fname">${esc(name)} <span class="muted">· ${lines.length} lines · ${text2.length} chars</span></div>` + `<pre><code>${lines.map((l, i) => `<span class="ln">${i + 1}</span>${esc(l)}`).join(`
`)}</code></pre>`;
}
function renderStatus() {
  $2("status").textContent = state.status;
  $2("status").className = "pill " + (state.status === "live" ? "live" : "idle");
  $2("peers").textContent = state.peers.length ? state.peers.join(", ") : "nobody else connected";
}
async function main() {
  const target = parseTarget();
  if (!target) {
    $2("app").innerHTML = '<div class="card warn">This URL is missing a session or key. Open a viewer link of the form <code>/s/&lt;owner&gt;/&lt;session&gt;#&lt;key&gt;</code>.</div>';
    return;
  }
  const token = localStorage.getItem(KEY);
  if (!token) {
    $2("app").innerHTML = '<div class="card">You need to sign in first. <a href="/dashboard">Go to the dashboard</a>, sign in, then reopen this link.</div>';
    return;
  }
  let roomKey;
  try {
    ...
    $2("app").innerHTML = `<div class="card warn">Bad room key in the link: ${esc(e.message)}</div>`;
    return;
  }
  $2("title").textContent = `${target.owner}/${target.session}`;
  ...
  provider.on("fatal", ({ reason }) => {
    $2("app").innerHTML = `<div class="card warn">${esc(reason)}</div>`;
  });
  try {
